...
export default function About() {
  const highlights = [
    { icon: <Code2 size={18} className="text-cyan-400 flex-shrink-0" />, label: "Full-Stack Development" },
    { icon: <Cpu size={18} className="text-blue-400 flex-shrink-0" />, label: "Machine Learning & AI" },
    { icon: <Server size={18} className="text-indigo-400 flex-shrink-0" />, label: "Scalable Microservices" },
    { icon: <Sparkles size={18} className="text-sky-400 flex-shrink-0" />, label: "Open-Source Contributor" },
  ];

  return (
    <section
      id="about"
      className="relative w-full py-16 sm:py-24 px-4 sm:px-6 bg-[#030712] text-slate-200 scroll-mt-16"
    >
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 items-center">
        ...
        >
          ...
          <div className="relative p-1.5 sm:p-2 bg-slate-900/90 border border-cyan-500/30 rounded-3xl shadow-2xl backdrop-blur-xl w-full max-w-[280px] sm:max-w-[360px] md:max-w-[420px]">
            <Image
              src="/image/about-img.png"
              width={420}
              height={420}
              alt="Priyanshu - Software Development Engineer"
              className="rounded-2xl object-cover shadow-xl brightness-95 w-full h-auto"
            />
          </div>
        </motion.div>
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-100 to-cyan-400 leading-tight">
